...
include_once '../templates/navbar-auditor.php';

// Get the auditor ID from session
$auditorID = $_SESSION["auditor_id"];

// Fetch auditor details
$auditorQuery = "SELECT * FROM Auditor WHERE AuditorID = ?";
$stmt = prepare($auditorQuery);
$stmt->bind_param("s", $auditorID);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    echo "<div class='alert alert-danger'>Auditor not found!</div>";
    exit();
}

$auditorData = $result->fetch_assoc();

// Fetch member details (since auditor is also a member)
$memberQuery = "SELECT * FROM Member WHERE MemberID = ?";
$stmt = prepare($memberQuery);
$stmt->bind_param("s", $auditorData['MemberID']);
$stmt->execute();
$memberResult = $stmt->get_result();
$memberData = $memberResult->num_rows > 0 ? $memberResult->fetch_assoc() : null;

// Fetch user details
$userQuery = "SELECT Username, Email, last_login FROM User WHERE Auditor_AuditorID = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["change_password"])) {
    ...
    $passwordQuery = "SELECT Password FROM User WHERE Auditor_AuditorID = ?";
    ...
    if (!$passwordCorrect) {
        ...
        // Update the password
        $updatePasswordQuery = "UPDATE User SET Password = ? WHERE Auditor_AuditorID = ?";
        $stmt = prepare($updatePasswordQuery);
        $stmt->bind_param("ss", $hashedPassword, $auditorID);
    ...
    <div class="profile-header">
        <div class="d-flex flex-column flex-md-row align-items-center">
            ...
            <div class="text-center text-md-start">
                <h2><?php echo htmlspecialchars($auditorData['Name']); ?></h2>
                <p class="mb-1"><i class="fas fa-id-card me-2"></i>Auditor ID: <?php echo htmlspecialchars($auditorData['AuditorID']); ?></p>
                <p class="mb-1"><i class="fas fa-calendar-alt me-2"></i>Term: <?php echo htmlspecialchars($auditorData['Term']); ?></p>
                <p><i class="fas fa-circle me-2"></i>Status: 
                    <span class="badge <?php echo ($auditorData['isActive'] == 1) ? 'bg-success' : 'bg-danger'; ?>">
                        <?php echo ($auditorData['isActive'] == 1) ? 'Active' : 'Inactive'; ?>
                    </span>
                </p>
            </div>
        </div>
            ...
            <div class="tab-content" id="profileTabContent">
                ...
                <div class="tab-pane fade show active" id="info-content" role="tabpanel" aria-labelledby="info-tab">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="info-card">
                                <h5><i class="fas fa-user me-2"></i>Auditor Details</h5>
                                
                                <div class="info-item">
                                    <div class="info-label">Full Name</div>
                                    <div class="info-value"><?php echo htmlspecialchars($auditorData['Name']); ?></div>
                                </div>
                                
                                <div class="info-item">
                                    <div class="info-label">Auditor ID</div>
                                    <div class="info-value"><?php echo htmlspecialchars($auditorData['AuditorID']); ?></div>
                                </div>
                                
                                <div class="info-item">
                                    <div class="info-label">Current Term</div>
                                    <div class="info-value"><?php echo htmlspecialchars($auditorData['Term']); ?></div>
                                </div>
                                ...
                                <div class="info-item">
                                    ...
                                    <div class="info-value"><?php echo htmlspecialchars($auditorData['MemberID']); ?></div>
                                </div>
